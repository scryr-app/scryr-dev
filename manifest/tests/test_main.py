...
# Add parent directory to path to import scryr
...
def test_discover_pydantic_instances_warns_on_broken_module() -> None:
    """Broken modules should warn to stderr when strict mode is disabled."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        broken_file = tmp_path / "broken.py"
        broken_file.write_text('raise RuntimeError("boom")')

        stderr = io.StringIO()
        results = discover_pydantic_instances(tmp_path, stderr=stderr)

        assert results == []
        assert "Warning: failed to load" in stderr.getvalue()
        assert "boom" in stderr.getvalue()


def test_discover_pydantic_instances_strict_mode_raises() -> None:
    """Strict mode should surface module import errors immediately."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        broken_file = tmp_path / "broken.py"
        broken_file.write_text('raise RuntimeError("boom")')
        ...
        with pytest.raises(RuntimeError, match="boom"):
            discover_pydantic_instances(tmp_path, strict=True)
